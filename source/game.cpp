  ...
  for(b2ContactEdge* ce = ball.body->GetContactList(); ce != NULL; ce = ce->next)
    {
      ...
      if(ce->contact->IsTouching())
      {
        ...
        if(filter.categoryBits & Filters::Bit_Water) //kolizja z wod¹
        {
			ballAtStartPos = true;
        }
        if(filter.categoryBits & Filters::Bit_Exit) //kolizja z do³kiem
        {
            mapState = (MapState)((mapState + 1)%3);
            ballAtStartPos = true;
        }
		if(filter.categoryBits & (Filters::Bit_Boundary | Filters::Bit_Wall_Obstacle)) //kolizja z map¹
        {
			b2WorldManifold wm;
			ce->contact->GetWorldManifold(&wm);
			createParticles(wm.points[0], 8);
        }
        if(filter.categoryBits & (Filters::Bit_Sensor_Obstacle)) //kolizja z map¹
        {
            /*b2Vec2 force = b2Vec2();
...
void Game::CreateBall()
{
    ...
    ball_fd.filter.maskBits = Filters::Bit_Exit | Filters::Bit_Wall_Obstacle | Filters::Bit_Boundary | Filters::Bit_Water | Filters::Bit_Sensor_Obstacle; //... pi³eczk¹
