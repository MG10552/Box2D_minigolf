...
void  Game::createMap1(int win_x, int win_y) {
    ...
    ////... nasz¹ pi³eczk¹
    fd.filter.maskBits = Filters::Bit_Ball | Filters::Bit_Particle; 
    b2ChainShape cs;
    ...
    // Wspó³czynnik odbicia - mno¿nik dla prêdkoœci odbicia po kolizji; 0 wygasza odbicie
    spfd.restitution = 0.0f; 
    spfd.isSensor = true;
    
    // Filtr kolizji
    //// Piasek kolidowaæ bêdzie z ...
    spfd.filter.categoryBits = Filters::Bit_Sensor_Obstacle;

    //... niczym / nasz¹ pi³eczk¹
    spfd.filter.maskBits = Filters::Bit_Ball; 
    b2PolygonShape sp;
    ...
    // Gêstoœæ materia³u - dla obiektu statycznego nie ma to znaczenia, dla obiektów dynamicznych - gêstoœæ*pole powierzchni = masa obiektu.
    bouncer_fd.density = 0.0f; 

    // Wspó³czynnik tarcia przy styku cia³ - w tym przyk³adzie mo¿e byæ dowolny
    bouncer_fd.friction = 0.0f;   

    // Wspó³czynnik odbicia - mno¿nik dla prêdkoœci odbicia po kolizji; 0 wygasza odbicie
    bouncer_fd.restitution = 2.0f; 
    
    // Filtr kolizji
    //// Boundary kolidowaæ bêdzie z ...
    bouncer_fd.filter.categoryBits = Filters::Bit_Wall_Obstacle;
    
    ////... particlami
    bouncer_fd.filter.maskBits = -Filters::Bit_Particle | Filters::Bit_Ball; 
    ...
    // Gêstoœæ materia³u - dla obiektu statycznego nie ma to znaczenia, dla obiektów dynamicznych - gêstoœæ*pole powierzchni = masa obiektu.
    hole1_fd.density = 0.0f; 

    // Wspó³czynnik tarcia przy styku cia³ - w tym przyk³adzie mo¿e byæ dowolny
    hole1_fd.friction = 0.0f;   

    // Wspó³czynnik odbicia - mno¿nik dla prêdkoœci odbicia po kolizji; 0 wygasza odbicie
    hole1_fd.restitution = 0.0f; 
    
    //Filtr kolizji
    //// Boundary kolidowaæ bêdzie z ...
    hole1_fd.filter.categoryBits = Filters::Bit_Exit;

    //... pi³eczk¹
    hole1_fd.filter.maskBits = Filters::Bit_Ball; 
    ...
    // Gêstoœæ materia³u - dla obiektu statycznego nie ma to znaczenia, dla obiektów dynamicznych - gêstoœæ*pole powierzchni = masa obiektu.
    hole2_fd.density = 0.0f; 

    // Wspó³czynnik tarcia przy styku cia³ - w tym przyk³adzie mo¿e byæ dowolny
    hole2_fd.friction = 0.0f;   

    // Wspó³czynnik odbicia - mno¿nik dla prêdkoœci odbicia po kolizji; 0 wygasza odbicie
    hole2_fd.restitution = 0.0f; 
    
    //Filtr kolizji
    //// Boundary kolidowaæ bêdzie z ...
    hole2_fd.filter.categoryBits = Filters::Bit_Exit;

    ////... pi³eczk¹
    hole2_fd.filter.maskBits = Filters::Bit_Ball; 
    ...
    // Wspó³czynnik odbicia - mno¿nik dla prêdkoœci odbicia po kolizji; 0 wygasza odbicie
    ff1fd.restitution = 0.0f; 
    ff1fd.isSensor = true;
    
    // Filtr kolizji
    //// Piasek kolidowaæ bêdzie z ...
    ff1fd.filter.categoryBits = Filters::Bit_Sensor_Obstacle; 
    
    ////... niczym / nasz¹ pi³eczk¹
    ff1fd.filter.maskBits = Filters::Bit_Ball; 
    b2PolygonShape ff1s;
    b2Vec2 ff1_verts[4] = {
        s2b * b2Vec2(0, 0),
        s2b * b2Vec2(0, -60),
    ...
    // Wspó³czynnik odbicia - mno¿nik dla prêdkoœci odbicia po kolizji; 0 wygasza odbicie  
    ff2fd.restitution = 0.0f; 
    ff2fd.isSensor = true;
    
    // Filtr kolizji
    //// Piasek kolidowaæ bêdzie z ...
    ff2fd.filter.categoryBits = Filters::Bit_Sensor_Obstacle; 
    ////... niczym / nasz¹ pi³eczk¹
    ff2fd.filter.maskBits = Filters::Bit_Ball; 
    b2PolygonShape ff2s;
    b2Vec2 ff2_verts[4] = {
        s2b * b2Vec2(0, 0),
        s2b * b2Vec2(0, -60),
    ...
    // Wspó³czynnik odbicia - mno¿nik dla prêdkoœci odbicia po kolizji; 0 wygasza odbicie  
    ff3fd.restitution = 0.0f; 
    ff3fd.isSensor = true;
    
    // Filtr kolizji
    //// Piasek kolidowaæ bêdzie z ...
    ff3fd.filter.categoryBits = Filters::Bit_Sensor_Obstacle; 
    
    ////... niczym / nasz¹ pi³eczk¹
    ff3fd.filter.maskBits = Filters::Bit_Ball; 
    b2PolygonShape ff3s;
    b2Vec2 ff3_verts[4] = {
        s2b * b2Vec2(0, 0),
        s2b * b2Vec2(0, -60),
    ...
    // Gêstoœæ materia³u - dla obiektu statycznego nie ma to znaczenia, dla obiektów dynamicznych - gêstoœæ*pole powierzchni = masa obiektu.
    bouncer2_fd.density = 0.0f; 
    
    // Wspó³czynnik tarcia przy styku cia³ - w tym przyk³adzie mo¿e byæ dowolny
    bouncer2_fd.friction = 0.0f; 
    
    // Wspó³czynnik odbicia - mno¿nik dla prêdkoœci odbicia po kolizji; 0 wygasza odbicie  
    bouncer2_fd.restitution = 2.0f; 
    ...
    // Gêstoœæ materia³u - dla obiektu statycznego nie ma to znaczenia, dla obiektów dynamicznych - gêstoœæ*pole powierzchni = masa obiektu.
    hole3_fd.density = 0.0f; 
    
    // Wspó³czynnik tarcia przy styku cia³ - w tym przyk³adzie mo¿e byæ dowolny
    hole3_fd.friction = 0.0f;   

    // Wspó³czynnik odbicia - mno¿nik dla prêdkoœci odbicia po kolizji; 0 wygasza odbicie
    hole3_fd.restitution = 0.0f; 
    
    // Filtr kolizji
    //// Boundary kolidowaæ bêdzie z ...
    hole3_fd.filter.categoryBits = Filters::Bit_Exit; 
    
    //... pi³eczk¹
    hole3_fd.filter.maskBits = Filters::Bit_Ball; 
...
void Game::CreateBall() {
    ...
    ////... pi³eczk¹
    ball_fd.filter.maskBits = Filters::Bit_Exit | Filters::Bit_Wall_Obstacle | Filters::Bit_Boundary | Filters::Bit_Water; 
